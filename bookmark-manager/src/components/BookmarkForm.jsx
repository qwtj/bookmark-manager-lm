...
const BookmarkForm = ({ bookmark, onClose, onSave, onDelete, fetchUrlStatus}) => {
  ...
  const llm = useMemo(() => {
    const provider = (typeof __llm_provider__ !== 'undefined' && __llm_provider__) || LLM_PROVIDERS.GEMINI;
    const options = (typeof __llm_options__ !== 'undefined' && __llm_options__) || {};
  ...
  const toCsvTags = (text) => {
    ...
    const seen = new Set();
    const unique = parts.filter(tag => (seen.has(tag.toLowerCase()) ? false : (seen.add(tag.toLowerCase()), true)));
    return unique.join(', ');
  };
  ...
  useEffect(() => {
    ...
    if (ignoreUrlValidation) return () => { isMounted = false; };

    // Show checking immediately when URL changes or is empty
    setCurrentUrlValidity('checking');

    if (!fetchUrlStatus) return () => { isMounted = false; };
  ...
  const handleRatingChange = (newRating) => {
    setFormData(prev => ({ ...prev, rating: prev.rating === newRating ? 0 : newRating }));
  };
  ...
  const generateDescriptionWithGemini = async () => {
    ...
    try {
      ...
      if (suggested) setFormData(prev => ({ ...prev, description: suggested }));
    } catch (error) {
  ...
  const generateTagsWithGemini = async () => {
    ...
    try {
      ...
      if (csv) setFormData(prev => ({ ...prev, tags: prev.tags ? `${prev.tags}, ${csv}` : csv }));
    } catch (error) {
      ...
      <div className="grid grid-cols-1 gap-4 mb-6">
        ...
        <div>
          ...
          <input
            ...
            required
          />
          {hasUrlInputChanged && (ignoreUrlValidation || currentUrlValidity !== 'valid') && (
            <button
              type="button"
